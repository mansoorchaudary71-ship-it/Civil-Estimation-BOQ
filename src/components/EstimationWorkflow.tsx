...
export default function EstimationWorkflow({ onSelectModule }: EstimationWorkflowProps) {
  ...
  return (
    <div className="w-full md:max-w-[1400px] md:mx-auto px-4 lg:px-8 mb-20 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-500">
      <div className="w-full bg-surface-default rounded-[2.5rem] p-4 sm:p-8 md:p-5 sm:p-12 border border-ui-borderSubtle shadow-xl overflow-hidden relative">
        ...
        <div className="w-full relative z-10 text-center md:max-w-2xl md:mx-auto mb-12 px-4 md:px-0">
          <h2 className="md: tabular-nums mb-4 text-xl font-semibold text-txt-primary tracking-tight">
            ...
            {steps.map((step, index) => (
              <div key={step.id} className="relative group cursor-pointer" onClick={() => onSelectModule(step.id as ModuleId)}>
                <div className="flex flex-col items-center text-center">
                  {/* Step Item Box */}
                  <div className={`w-28 h-28 lg:w-32 lg:h-32 rounded-2xl bg-surface-default  border-2 ${step.border} shadow-lg flex flex-col items-center justify-center transition-transform duration-300 group-hover:-translate-y-2 group-hover:shadow-2xl relative mb-6 z-10`}>
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${step.color} mb-2`}>
                      <step.icon className="w-6 h-6" />
                    </div>
                    {/* Number Badge */}
                    <div className="w-full absolute -top-4 -right-4 w-9 h-9 bg-surface-default text-txt-primary font-semibold tabular-nums tracking-tight rounded-full flex items-center justify-center shadow-md text-sm border-4 border-white overflow-hidden">
                      {step.number}
                    </div>
                  </div>

                  <h3 className="mb-2 group-hover:text-purple-600 transition-colors text-lg font-medium text-txt-primary mb-4">
                    {step.title}
                  </h3>
                  <p className="px-4 text-base font-normal text-txt-secondary leading-relaxed">
                    {step.description}
                  </p>
                ...
                {index < steps.length - 1 && (
                  <div className="hidden lg:flex absolute top-[calc(4rem-0.75rem)] -right-6 w-12 h-6 items-center justify-center z-20 text-txt-secondary group-hover:text-purple-500 group-hover:translate-x-1 transition-all duration-300">
