      ...
      <div className="bg-surface-default/80 backdrop-blur-xl rounded-2xl border border-ui-borderSubtle p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-txt-primary">Topsoil & Landscaping Estimator</h1>
              <p className="text-txt-secondary text-sm">Calculate soil volume, weight, bags, and truckloads.</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-surface-default/80 backdrop-blur-xl rounded-2xl border border-ui-borderSubtle p-6 shadow-sm space-y-6">
            ...
            {inputMode === 'dimensions' ? (
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-txt-secondary">Width ({unitL})</label>
                  <input
                    type="number"
                    min="0.1"
                    value={width}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-txt-secondary">Total Surface Area ({unitA})</label>
                <input
                  type="number"
                  min="0.1"
                  value={totalArea}
            ...
            <div className="pt-4 border-t border-slate-100">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-txt-secondary">Desired Depth ({unitD})</label>
                  <input
                    type="number"
                    min="0.1"
                    value={depth}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-txt-secondary">Compaction Factor (%)</label>
                  <input
                    type="number"
                    min="0"
                    value={compactionFactor}
                    onChange={(e) => setCompactionFactor(Math.max(0, Number(e.target.value)))}
                    className="w-full bg-slate-50 border border-ui-borderSubtle rounded-xl px-4 py-3 text-txt-primary focus:border-orange-500 outline-none"
                  />
                  <p className="text-[10px] text-txt-tertiary">Extra material needed due to settling.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-surface-default/80 backdrop-blur-xl rounded-2xl border border-ui-borderSubtle p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-txt-secondary">Material Type</label>
                <select 
                  value={soilType}
                  onChange={(e) => setSoilType(e.target.value as keyof typeof soilTypes)}
                  className="w-full bg-slate-50 border border-ui-borderSubtle rounded-xl px-4 py-3 text-txt-primary focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                >
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-txt-secondary">Bulk Density (kg/m³)</label>
                <input 
                  type="number" 
                  min="100" 
                  value={customDensity} 
                  onChange={(e) => setCustomDensity(Math.max(100, Number(e.target.value)))} 
                  className="w-full bg-slate-50 border border-ui-borderSubtle rounded-xl px-4 py-3 text-txt-primary focus:border-emerald-500 outline-none" 
                />
              </div>
            </div>
          </div>
            ...
            {result && (
              <div className="space-y-4 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-surface-default/5 rounded-2xl p-3 border border-white/5">
