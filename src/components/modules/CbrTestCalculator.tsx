          ...
          <div className="w-full md:w-[45%] flex flex-col gap-6">
            <div className="w-full bg-surface-default p-4 sm:p-6 rounded-2xl border border-ui-borderSubtle shadow-sm relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
                <h3 className="text-lg font-medium text-txt-primary mb-4">Test Readings</h3>
...
<input id="a11y-input-174"
                      ...
                      <div className="col-span-2 flex justify-end">
                        <Button aria-label="Delete"
                          ...
                          className="p-2 text-txt-secondary hover:text-red-500 hover:bg-red-50 rounded-full transition-colors active:scale-95 hover:-translate-y-0.5 hover:shadow-lg shadow-sm"
                        >
              ...
              <div className="mt-4 pt-4 border-t border-slate-100">
                 <label className="uppercase block mb-2 text-sm font-medium text-txt-secondary mb-1"><span className="flex items-center">
...
<input id="a11y-input-175"
                     ...
                     className="w-full max-w-[200px] bg-slate-50 rounded-full border border-ui-borderSubtle shadow-sm text-txt-primary border border-ui-borderSubtle text-txt-primary rounded-full px-4 py-2 font-mono text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none focus:ring-indigo-500 font-bold"
                   /></>
                   <span className="absolute left-[170px] top-1/2 -translate-y-1/2 text-txt-secondary text-base font-medium">%</span>
                 </div>
              </div>
          ...
          <div className="w-full md:w-[55%]">
            {isProcessing ? (
              ...
              <div className="space-y-6">
                <div className={`p-6 md:p-8 rounded-2xl border shadow-lg relative overflow-hidden transition-all duration-500 ${estimateData.passed ? 'bg-gradient-to-br from-emerald-50 to-teal-50/30 border-emerald-200   ' : 'bg-gradient-to-br from-rose-50 to-red-50/30 border-rose-200   '}`}>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-ui-borderDefault/5 pb-6 mb-6">
                    <div>
                      <span className="text-base font-medium uppercase tracking-widest text-txt-tertiary mb-1 block">Final Design CBR</span>
                    ...
                    <div className="flex shrink-0">
                      <span className={`px-4 py-2 rounded-2xl text-base font-medium shadow-sm uppercase tracking-wider border ${estimateData.passed ? 'bg-emerald-500 text-txt-primary border-emerald-600' : 'bg-rose-500 text-txt-primary border-rose-600'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="w-full bg-surface-default/60 p-4 rounded-2xl border border-ui-borderDefault/5 shadow-sm overflow-hidden">
                      <span className="text-txt-tertiary text-base font-medium uppercase tracking-widest block mb-1">CBR @ 2.5mm</span>
                      <div className="text-xl font-semibold text-txt-primary tabular-nums tracking-tight text-txt-primary">{estimateData.cbr25.toFixed(1)}%</div>
                      <span className="text-sm text-txt-secondary font-mono mt-1 block">Load: {estimateData.load25.toFixed(1)} kg</span>
                    </div>
                    <div className="w-full bg-surface-default/60 p-4 rounded-2xl border border-ui-borderDefault/5 shadow-sm overflow-hidden">
                      <span className="text-txt-tertiary text-base font-medium uppercase tracking-widest block mb-1">CBR @ 5.0mm</span>
                      <div className="text-xl font-semibold text-txt-primary tabular-nums tracking-tight text-txt-primary">{estimateData.cbr50.toFixed(1)}%</div>
                      <span className="text-sm text-txt-secondary font-mono mt-1 block">Load: {estimateData.load50.toFixed(1)} kg</span>
                    </div>
                  </div>
                  
                  <div className="w-full bg-surface-default rounded-2xl border border-ui-borderDefault/5 shadow-sm p-5 flex items-center justify-between mb-8 overflow-hidden">
                     <div>
                       <span className="block text-base font-medium uppercase tracking-widest text-txt-tertiary">Soil Quality Index</span>
                       <span className="text-lg font-semibold tabular-nums tracking-tight text-txt-primary">{estimateData.soilClass}</span>
                     </div>
                     <Activity className="w-8 h-8 text-txt-secondary" />
                  </div>

                  {/* Chart section */}
                  <div className="bg-surface-default p-5 rounded-2xl border border-ui-borderDefault/5 shadow-sm h-[300px] w-full pt-6 overflow-hidden">
                     <h3 className="text-sm uppercase st mb-4 text-lg font-medium text-txt-primary">Load vs. Penetration Curve</h3>
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-slate-50 rounded-2xl border border-ui-borderSubtle shadow-sm text-txt-primary rounded-2xl border-2 border-dashed border-ui-borderSubtle p-4 sm:p-8 md:p-8 text-center bg-graph-pattern opacity-80 mix-blend-multiply overflow-hidden">
                ...
                <h3 className="mb-2 text-lg font-medium text-txt-primary mb-4">Ready to Plot Load-Penetration</h3>
                ...
                <div className="w-full flex gap-3 text-base font-medium text-indigo-500 uppercase tracking-widest bg-surface-default px-4 py-2 rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
                   <span>2.5mm</span>
