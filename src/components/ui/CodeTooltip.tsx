...
export function CodeTooltip({ standard, code, description }: CodeTooltipProps) {
  ...
  const getStandardColor = (std: string) => {
    ...
    return 'bg-gray-100 text-txt-secondary border-gray-200';
  };
      ...
      <AnimatePresence>
        {show && (
          <motion.div
            ...
            className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-64 p-3 bg-surface-default dark:bg-slate-800 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] border border-ui-borderSubtle dark:border-slate-700 z-[100] text-left pointer-events-none"
          >
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-txt-secondary dark:text-slate-300 font-normal leading-relaxed">
                {description}
              </p>
            </div>
            
            {/* Arrow */}
            <div className="absolute left-1/2 -bottom-1.5 -translate-x-1/2 w-3 h-3 bg-surface-default dark:bg-slate-800 border-b border-r border-ui-borderSubtle dark:border-slate-700 transform rotate-45"></div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
