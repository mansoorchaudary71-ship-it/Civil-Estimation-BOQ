...
function StepCard({ step, idx }: { step: CalcStep; idx: number }) {
  ...
  return (
    <div className="w-full bg-surface-default/80 dark:bg-slate-800/80 rounded-2xl p-5 sm:p-4 sm:p-4 sm:p-4 sm:p-6 shadow-sm border border-ui-borderSubtle dark:border-slate-700/50 overflow-hidden">
      <h4 className="text-lg sm:text-xl font-bold text-txt-primary dark:text-slate-100 mb-5 flex items-center gap-3">
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h5 className="text-base font-medium uppercase tracking-wider text-slate-400 dark:text-txt-tertiary flex items-center gap-2">
          ...
          <div className="bg-surface-default dark:bg-slate-800/50 rounded-xl p-4 border border-ui-borderSubtle dark:border-slate-700 overflow-x-auto text-sm sm:text-base font-mono font-semibold text-txt-secondary dark:text-slate-300">
            {step.equation}
          </div>
        </div>
        ...
        {step.variables && step.variables.length > 0 && (
          <div>
            <h5 className="text-base font-medium uppercase tracking-wider text-slate-400 dark:text-txt-tertiary mb-2">
              Input Variables
            </h5>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              {step.variables.map((v, vIdx) => (
                <div
                  key={vIdx}
                  className="flex justify-between items-center px-4 py-3 bg-surface-default dark:bg-slate-800/50 rounded-xl border border-ui-borderSubtle dark:border-slate-700"
                >
                  <span className="text-sm font-medium text-txt-secondary dark:text-slate-400">
                    {v.name}
                  </span>
                  <div className="text-sm">
                    ...
                    {v.unit && (
                      <span className="text-slate-400 dark:text-txt-tertiary font-bold ml-1">
                        {v.unit}
                      </span>
                    )}
                  </div>
        ...
        {step.insight && showInsight && (
          <div className="flex gap-3 p-4 sm:p-5 rounded-2xl bg-indigo-50/70 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/30 animate-in fade-in slide-in-from-top-2 duration-300">
        ...
        <div className="bg-slate-800 dark:bg-slate-900/40 rounded-2xl p-4 sm:p-5 text-white">
...
export function DetailedCalculationDisplay({ 
  ...
  return (
    <div className={`w-full max-w-4xl mx-auto mt-8 mb-4 font-sans ${className}`}>
      <div className="w-full bg-surface-default/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(15,23,42,0.08)] border border-white/40 dark:border-slate-800/60 overflow-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-4 sm:gap-5">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center text-white shrink-0 shadow-lg shadow-indigo-500/30">
              <Calculator className="w-6 h-6 sm:w-7 sm:h-7" strokeWidth={2.5} />
            </div>
            <div>
              <h3 className="text-xl sm:text-2xl font-extrabold tracking-tight text-txt-primary dark:text-slate-100">
                {title}
              </h3>
              <p className="text-sm sm:text-base font-medium text-txt-tertiary dark:text-slate-400 mt-1">
                {subtitle}
              </p>
            </div>
          </div>
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center bg-surface-default dark:bg-slate-800 text-txt-tertiary dark:text-slate-400 transition-transform duration-300 shrink-0">
