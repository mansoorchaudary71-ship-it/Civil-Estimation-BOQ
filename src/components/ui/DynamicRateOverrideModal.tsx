...
export function DynamicRateOverrideModal({ isOpen, onClose, currentRates, onSave }: DynamicRateOverrideModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-surface-default dark:bg-slate-900 rounded-2xl shadow-2xl border border-ui-borderSubtle dark:border-slate-800 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800">
          <h2 className="text-xl font-bold text-txt-primary dark:text-white">Material Rates Override</h2>
          <Button 
            onClick={onClose}
            className="p-2 rounded-full text-slate-400 hover:text-txt-secondary hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            <X className="w-5 h-5" />
          </Button>
        </div>
...
function RateField({ label, value, onChange }: { label: string, value: number, onChange: (val: string) => void }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <label className="text-sm font-medium text-txt-secondary dark:text-slate-300 w-1/2">{label}</label>
      <div className="w-1/2 relative">
        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-medium">Rs</span>
        <MaskedInput
          value={value}
          onValueChange={onChange}
          className="w-full bg-slate-50 dark:bg-slate-800 border border-ui-borderSubtle dark:border-slate-700 rounded-xl py-2 pl-9 pr-4 text-right text-txt-primary dark:text-white font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 focus:outline-none focus:ring-indigo-500/50 focus:border-indigo-500 transition-all"
        />
      </div>
    </div>
  );
