      ...
      <div className="w-full max-w-2xl bg-surface-default/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-2xl sm:rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] border border-white/40 dark:border-slate-800 overflow-hidden flex flex-col animate-in slide-in-from-top-8 sm:zoom-in-95 duration-300">
        ...
        <div className="p-6 sm:p-8 flex items-center gap-4 border-b border-slate-100/50 dark:border-slate-800/50">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none text-xl sm:text-2xl font-medium text-txt-primary dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-txt-tertiary focus:ring-0 tracking-tight"
          />
          <Button 
            onClick={onClose}
            className="p-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-2xl text-slate-400 hover:text-txt-secondary dark:hover:text-slate-200 transition-colors shrink-0"
          >
        ...
        <div className="flex-1 overflow-y-auto hide-scrollbar max-h-[60vh]">
          {searchWords.length === 0 ? (
            <div className="p-12 text-center flex flex-col items-center justify-center">
              <div className="w-20 h-20 rounded-2xl bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center mb-5 shadow-sm border border-slate-100 dark:border-slate-800">
                <Search className="w-8 h-8 text-slate-300 dark:text-txt-secondary" strokeWidth={2} />
              </div>
              <p className="text-txt-tertiary dark:text-slate-400 font-medium text-lg tracking-tight">Type anything to start searching...</p>
            </div>
          ) : results.length > 0 ? (
            <div className="p-4 sm:p-6 flex flex-col gap-3">
              <div className="px-3 pb-2">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-txt-tertiary">
              ...
              {results.map((mod) => (
                <Button
                  ...
                  className="group flex items-start gap-5 p-4 sm:p-5 rounded-2xl bg-slate-50/50 hover:bg-indigo-50/50 dark:bg-slate-800/30 dark:hover:bg-indigo-900/20 border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800/50 transition-all duration-300 text-left w-full hover:-translate-y-1 hover:shadow-lg hover:shadow-indigo-500/5"
                >
                  <div className="w-14 h-14 rounded-2xl bg-surface-default dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-center shrink-0 text-txt-tertiary group-hover:text-indigo-600 dark:group-hover:text-indigo-400 group-hover:scale-110 group-hover:-rotate-3 transition-all duration-300">
                    <mod.icon className="w-6 h-6" strokeWidth={1.5} />
                  </div>
                  <div className="flex-1 overflow-hidden pt-1">
                    <h3 className="text-lg font-semibold text-txt-primary dark:text-slate-100 mb-1.5 group-hover:text-indigo-700 dark:group-hover:text-indigo-400 transition-colors tracking-tight">
                      {highlightMatch(mod.title, searchWords)}
                    </h3>
                    <p className="text-sm text-txt-tertiary dark:text-slate-400 leading-relaxed line-clamp-2 font-normal">
            ...
            <div className="p-12 text-center flex flex-col items-center justify-center">
               <div className="w-20 h-20 rounded-2xl bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center mb-5 shadow-sm border border-slate-100 dark:border-slate-800">
                <Search className="w-8 h-8 text-slate-300 dark:text-txt-secondary opacity-50" strokeWidth={2} />
              </div>
              <p className="text-txt-secondary dark:text-slate-300 font-semibold text-lg tracking-tight">No results found for "{searchTerm}"</p>
              <p className="text-txt-tertiary dark:text-slate-400 text-sm mt-2 font-normal">Try searching for materials like "concrete", "steel", or tools like "calculator".</p>
            </div>
          )}
        </div>
      </div>
